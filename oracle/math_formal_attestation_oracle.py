#!/usr/bin/env python3
"""math-formal-attestation-oracle -- a deterministic attestation-integrity decider for the `math-formal` knowledge domain.

An **oracle** *deterministically decides* the truth of a case. This one decides, for a `math-formal` knowledge
capsule, whether it is a well-formed, provenance-carrying attestation. VALID iff: domain=='math-formal', a non-empty
...
def decide(c):
    if c.get("domain") != "math-formal": return "INVALID"
...
def _run_probes(path):
    ...
    print("math-formal attestation oracle: probes=%d | recall=%.3f | false_positives=%d | verdict=%s"%(len(probes),recall,fp,"PASS" if ok else "FAIL"))
